...
Reads from the HDF5 files generated by generator.py and provides:
  - Full RAM caching on first load (eliminates repeated HDF5 disk I/O)
...
class IsingDataset(Dataset):
    ...
    All arrays are loaded into RAM on construction (cache=True by default).
    This eliminates the per-sample HDF5 open/read/close overhead that was
    ...
    cache      : load all arrays into RAM at init time (strongly recommended)
    """
    ...
    def __init__(
        ...
        # Load metadata and optionally all data into RAM
        with h5py.File(self.h5_path, "r") as hf:
            ...
            if cache:
                # Load everything once — eliminates all future disk I/O
                self._cnn_input    = hf["cnn_input"][:].astype(np.float32)      # (N, 4, L, L)
                self._phase        = hf["phase"][:].astype(np.int64)            # (N,)
                self._temperature  = hf["temperature"][:].astype(np.float32)    # (N,)
                self._magnetization= hf["magnetization"][:].astype(np.float32)  # (N,)
                self._energy       = hf["energy"][:].astype(np.float32)         # (N,)
                self._cached       = True
            else:
                self._cached = False
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        if self._cached:
            # Zero-copy slice from RAM — no disk I/O
            cnn_input = self._cnn_input[raw_idx].copy()
            phase     = int(self._phase[raw_idx])
            T         = float(self._temperature[raw_idx])
            m         = float(self._magnetization[raw_idx])
            E         = float(self._energy[raw_idx])
        else:
            # Fallback: open HDF5 (slow path, kept for large datasets that don't fit in RAM)
            with h5py.File(self.h5_path, "r") as hf:
...
def make_dataloaders(
    ...
    augment: bool = False,          # keep off initially for speed; enable after convergence
    spin_flip: bool = False,        # same: off by default for speed
    num_workers: int = 4,           # 4 paralel worker → disk I/O darboğazını kaldır
    cache: bool = True,             # load full dataset to RAM
    seed: int = 42,
    ...
    Training set:
      - Full RAM caching (cache=True) eliminates HDF5 I/O overhead
      - Symmetry augmentation optional (off by default for speed)
    ...
    print(f"  Loading dataset into RAM (cache=True)... ", end="", flush=True)
    train_ds = IsingDataset(h5_path, train_idx, augment=augment, spin_flip=spin_flip, cache=cache)
    val_ds   = IsingDataset(h5_path, val_idx,   augment=False,   spin_flip=False,     cache=cache)
    test_ds  = IsingDataset(h5_path, test_idx,  augment=False,   spin_flip=False,     cache=cache)
    ...
    # cache=True durumunda num_workers=0 bile hızlıdır (RAM'den okuma), ama
    # physics hesabı (chi, beta) CPU'da yapıldığından birkaç worker hâlâ faydalı.
